generateHist <- function(mutTable, normalise=TRUE, pyr.transcribed=NA) {
    ...
    if(nrow(mutTable)>0) {
        ...
        if (length(pyr.transcribed)==1) {
            mut.context.table <- as.data.frame(table(mutTable$mut.context)) # count all occurences of each mutation type

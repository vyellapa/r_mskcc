SIGN.CONTEXT.FILE = '/ifs/work/leukgen/home/gg10/soft/integrated.circos/data/signatures_probabilities.txt'
sh <- read.table(SIGN.CONTEXT.FILE, header=TRUE, sep='\t')
mut.order <- (sh[,'Somatic.Mutation.Type'])


getMutTables <- function(myFile, onlyPASSED=FALSE, onlyPassedAsrd=FALSE, genome.v="hg19", genomeSeq=Hsapiens, addContext=TRUE, normalise=FALSE) {
...
# the mutations originally not in pyramidine contex
    ...
    all.hist <- generateHist(mut.table, normalise=normalise)
    passed.hist <- generateHist(mut.table[fs.passed,], normalise=normalise)
    if (sum(info.data[,'SNP'])>0) {
        snps.hist <-  generateHist(mut.table[info.data[,'SNP'],], normalise=normalise)
        names(snps.hist) <- mut.order
    } else {
        snps.hist <- NULL
    }
    non.snps.hist <-  generateHist(mut.table[!info.data[,'SNP'],], normalise=normalise)
...
getMutTablesCustom <- function(myFile, onlyPASSED=FALSE, genome.v="hg19", genomeSeq=Hsapiens, normalise=TRUE, addContext=TRUE, sign.context.file=SIGN.CONTEXT.FILE) {

                                        # plots mutation-context for all variants in the vcf file
                                        # and separately for the variants that passed

# Read the signature context file
sh <- read.table(sign.context.file, header=TRUE, sep='\t')
mut.order <- (sh[,'Somatic.Mutation.Type'])
# load the vcf file    
B35.1.2.vcf <- readVcf(myFile, genome.v)

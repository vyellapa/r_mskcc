...
parser$add_argument("-a", "--subs", default=NULL, type="character", nargs=1, help="Optional. Absolute path to the VCF file for substitutions.")
parser$add_argument("-p", "--pos", default="none", type="character", nargs=1, help="Optional. Genomic window to zoom in: chr_start_end.")
parser$add_argument("-d", "--chrs", default="none", type="character", nargs=1, help="Optional. To plot multiple chromosomes in a single image. Chr names separated by commas.")
...
parser$add_argument("-z", "--facet", action="store_true", default=FALSE, help="Optional. Facet by levels in the category column in the bedpe file. Only pass bedpe and cn_bedgrap files.")
...
parser$add_argument("-g", "--girth", default=9, type="double", nargs=1, help="Optional .Width of the image.")
	...
	for (i in 1:nrow(bb_sub)) {
		if(is_bbg) {
			...
			if(!is.na(bb_sub[i,"nMaj2_A"])) {
				...
				paste(bb_sub[i,"nMaj2_A"],"+",bb_sub[i,"nMin2_A"],": ",100*round(as.numeric(bb_sub[i,"frac2_A"]),3),"%",sep=""), cex = 1.5)
			}
		} else {
			text((as.numeric(bb_sub[i,"startpos"])+as.numeric(bb_sub[i,"endpos"]))/2, as.numeric(bb_sub[i,"BAF"])-0.04, paste('TCN=',bb_sub[i,"cn"],sep=""), cex=1.5)
...
image_width = args$girth

bedpe_file ='triple_callers_filtered_manual.bedpe' 
bg_file = '/work/isabl/home/gundemg/projects/neuroblastoma/samples/E-H-131239-T1-1-D1-1/brass/E-H-131239-T1-1-D1-1.ngscn.abs_cn.bg.gz'
bb_file = '/work/isabl/home/gundemg/projects/neuroblastoma/samples/E-H-131239-T1-1-D1-1/bbg/E-H-131239-T1-1-D1-1_subclones.txt'
baf_file = '/work/isabl/home/gundemg/projects/neuroblastoma/samples/E-H-131239-T1-1-D1-1/bbg/E-H-131239-T1-1-D1-1_logR_Baf_segmented.vcf.gz'
legend_pos = NULL
rearr_col = 'type'
bbg_col = 'custom'
image_type = 'pdf'
...
subs_file = NULL
facet = FALSE
output_file_stub = 'E-H-131239-T1-1-D1-1'
flip_higher = FALSE
image_width = 9
coord = "2_13000000_17000000"
#coord = "none"; plot_chrs = "7,12"
# Read substitutions from the VCF file
# readVCF does not work properly if I read the VCF file before i source the plotting script
substitutions = NULL
...
if(count>0) {
	if(headered==0) {
		...
		if(sum(grepl('chr1',brass[1,])+0)>0) {
			brass = read.table(file=bedpe_file, sep='\t', stringsAsFactors=F, header=T, comment.char="#")
		}
...
if(coord == 'none') {
	chrs_to_plot = c()
	if(plot_chrs!='none') {
		chrs_to_plot = unlist(strsplit(plot_chrs, split=","))
	...
	if(plot_chrs=='none') {
		for(chr in chrs_to_plot) {
			...
			if(!is.null(baf_data) & chr!='Y') {
				par(mfrow=c(2,1), mar=c(0.5, 0.5, 0.2, 0.2))
...
if(!is.null(subs_file)) {
	...
	axis(2, at=axisTicks(usr=c(0,8), log=F), las=2)
	title(ylab="log10(inter-mutation distance)")
	points(x=substitutions[selected_subs2,'starts'], y=log10(substitutions[selected_subs2,'intermut_dist']), pch=16, col=substitutions[selected_subs2,'color'])
	...
	axis(2, at=axisTicks(usr=c(0,8), log=F), las=2)
	title(ylab="log10(inter-mutation distance)")
	points(x=substitutions[selected_subs1,'starts'], y=log10(substitutions[selected_subs1,'intermut_dist']), pch=16, col=substitutions[selected_subs1,'color'])
...
if(coord != "none" & is.null(subs_file) & !facet){
	height = 9
	if(!is.null(baf_data)) {height=12}
	if(!grepl('chrs:',coord)) {
		if(image_type == 'pdf') {
...
#			par(mfrow=c(2,1), xpd=TRUE)
	...
	dev.off()
} else if (coord !="none" & is.null(subs_file) & facet) {
	chrs_to_plot = c(chr)
	coord = gsub(',','_',gsub(':','',coord))
	bedpe=bedpe[((bedpe$m_chrl==chr & bedpe$m_l5>start & bedpe$m_l3<end) | (bedpe$m_chrh==chr & bedpe$m_h5>start & bedpe$m_h3<end)),]

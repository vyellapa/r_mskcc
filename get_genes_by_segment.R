...
if(cntype=='brass') {
	...
	segments = rbind(segments[segments$ntot>ploidy,], segments[segments$ntot<2,])
	gr.segments = GRanges(
		seqnames=Rle(sub('chr','', segments$chr)), IRanges(segments$startpos, segments$endpos),
		abs_cn=segments$ntot, baf=segments$BAF, logr=segments$LogR
	)

...
#heatmap.2(as.matrix(clust), dendrogram="row",Rowv=TRUE,Colv=FALSE, scale="row",distfun=function(clust) dist(clust,method = 'euclidean'), hclustfun= function(clust) hclust(clust,method = 'ward'),colsep=c(1), RowSideColors= sidecols, col = bluered, trace="none")
#dev.off()

png(file = "d15.png", res=300, width=2000, height=3000)
heatmap.2(as.matrix(clust), dendrogram="col",Rowv=TRUE,Colv=FALSE, scale="col",distfun=function(clust) dist(clust,method = 'euclidean'), hclustfun= function(clust) hclust(clust,method = 'ward'),colsep=c(1), ColSideColors= sidecols, col = bluered, trace="none")

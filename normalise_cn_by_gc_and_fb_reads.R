...
# gg10 START  9Feb2016    I am changing diagnostic plots since I would like to see data for all the chromosomes.
pdf(paste0(output_body, ".diagnostic_plots.pdf"))
smoothScatter(gc, logratio, pch = ".", xlab = "GC-content", ylab = "Log-ratio")
smoothScatter(tumour_fb_ratio, logratio, pch = ".", xlab = "Tumour FB ratio", ylab = "Log-ratio")
smoothScatter(normal_fb_ratio, logratio, pch = ".", xlab = "Normal FB ratio", ylab = "Log-ratio")
plot.Gam(gam_m, rugplot = F)
dev.off()
pdf(paste0(output_body, ".perchr.diagnostic_plots.pdf"), width=12)
for(chr in chrs) {
    par(mfrow=c(1,2))
    idx = d.t[,1] == chr
    plot(d.t[idx, 3], d.t[idx, 6], xlab = paste("Chr ", chr, " position", sep=""), ylab = "Read depth", pch = ".")
    plot(d.t[idx, 3], normalised_logratio[idx], xlab = paste("Chr ", chr, " position", sep=""), ylab = "Normalised log-ratio", pch = ".")	
}
dev.off()
# gg10 END  9Feb2016    I am changing diagnostic plots since I would like to see data for all the chromosomes.

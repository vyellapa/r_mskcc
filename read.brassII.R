read.brassII <- function(FILE.REARR) {
    ...
    if (!inherits(rearrs, "try-error")) {
        ...
        f.1 <- rearrs$V1==rearrs$V4 & rearrs$V9=='+' & rearrs$V10=='+'; pf[f.1] <- 2 # deletion ++ mapping direction
        f.8 <- rearrs$V1==rearrs$V4 & rearrs$V9=='-' & rearrs$V10=='+'; pf[f.8] <- 8 # inversion -+ mapping direction
        f.2 <- rearrs$V1==rearrs$V4 & rearrs$V9=='+' & rearrs$V10=='-'; pf[f.2] <- 1 # inversion +- mapping direction
        rearrs$pf <- pf

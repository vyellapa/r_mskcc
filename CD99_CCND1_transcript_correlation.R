...
library("biomaRt")
setwd("~/Desktop")
trans=read.table("~/Desktop/record/MMRF/MMRF_CoMMpass_IA9b_LongInsert_Canonical_Ig_Translocations.txt", header=T, sep="\t")
cyto=read.table("~/Desktop/record/MMRF/MMRF_CoMMpass_IA9b_CNA_LongInsert_FISH_CN_Unique_Visits.txt", header=T, sep="\t")
#maf=read.table("~/Desktop/MMRF/MMRF_Canonical_GLFilter_uniq.maf", sep = "\t", stringsAsFactors=FALSE, encoding= "utf-8")

salmon=read.table("~/Desktop/record/MMRF/MMRF_CoMMpass_IA9_E74cDNA_Salmon_Transcript_Counts.txt", header=T, sep="\t",stringsAsFactors=FALSE)

fpkm=read.table("~/Desktop/record/MMRF/MMRF_CoMMpass_IA9_E74GTF_Cufflinks_Gene_FPKM.txt", header=T, sep="\t",stringsAsFactors=FALSE)
counts=read.table("~/Desktop/record/MMRF/MMRF_CoMMpass_IA9_E74GTF_HtSeq_Gene_Counts.txt", header=T, sep="\t",stringsAsFactors=FALSE)
...
############### Salmon #########################
salmon$maxVal=apply(salmon[,2:dim(salmon)[2]], 1, max)
salmon=salmon[salmon$maxVal>20, ]
salmon=salmon[,-dim(salmon)[2]]

rownames(salmon)=salmon[,1]
salmon=salmon[,-1]
salmon=round(salmon)
...
## DESEQ
dsalmonNeg=salmon[,colnames(salmon) %in% tNegId ]
dsalmonNeg=dsalmonNeg[,1:10]
dsalmonPos=salmon[,colnames(salmon) %in% tPosId ]
dsalmonPos=dsalmonPos[,1:10]
n=rep("NEG",ncol(dsalmonNeg))
p=rep("POS",ncol(dsalmonPos))
conds=c(n,p)
dsalmonAll=cbind(dsalmonNeg,dsalmonPos)
dsalmonDesign=data.frame(row.names = colnames(dsalmonAll), condition=conds, libType = c(rep("paired",ncol(dsalmonAll))))
salmonTable=dsalmonAll
condition=dsalmonDesign$condition

library( "DESeq" )
cds = newCountDataSet( salmonTable, condition )
cds = estimateSizeFactors( cds )
#cds=counts(cds,normalized=TRUE) 
cds=estimateDispersions(cds)
res=nbinomTest(cds, "NEG", "POS")
write.table(res,file="~/Desktop/record/MM_CD99/mm_cd99_results_deseq/CCND1_transcript_deseq.tsv", append=T, quote=F,sep="\t", eol="\n", row.names=F, col.names=F)
...
ggplot(melted_plot, aes(variable, as.numeric(as.character(value)),fill=CASE))+geom_boxplot(alpha=0.6)+geom_point(alpha=0.3, position = position_dodge(width=0.75))+scale_fill_brewer(palette = "Set1")+theme_bw()+ylab("log 2 FPKM")+xlab("")

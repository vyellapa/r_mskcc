...
getBAFsAndLogRs<-function(imputeInfoFile,inputFile, normalInputFile,  BAFnormalFile, BAFmutantFile, logRnormalFile, logRmutantFile,minCounts=NA,samplename="sample1",thougenloc)
{
	...
	for(chr in 1:length(chr_names)){
		...
		tum = read.table(chr_file,header=TRUE,sep="\t",comment.char="")
		norm = read.table(normal_chr_file,header=TRUE,sep="\t",comment.char="")

		input_data<-rbind(input_data,tum)
		normal_input_data<-rbind(normal_input_data,norm)

		#allele_data<-rbind(allele_data,read.table(paste("1000genomesloci/1000genomesAlleles2012_chr",chr,".txt",sep=""),sep="\t",header=T))
		thousand = read.table(paste(thougenloc,"/1000genomesAlleles2012_chr",chr,".txt",sep=""))
		allele_data<-rbind(allele_data,thousand[thousand$V1 %in% tum$POS,])
	}

...
annotate_clinical <-function(clinical,
    ...
    ## Add sample purity
    purity <- somatic[!duplicated(somatic$LEUKID),]
    clinical <- left_join(clinical, 
                          purity[c("LEUKID", "PURITY")], 
                          by = 'LEUKID')
    
    ## Prepare gene variable
    somatic <- mutate(somatic, variant = paste(VAG_GENE, VAG_PROTEIN_CHANGE, sep = '_'))

...
parser$add_argument("-z", "--typez", default="default", type="character", nargs=1, help="Optional. Coloring scheme to use for Battenberg results. Use 'custom' for custom coloring.")
...
########################################################################
#bedpe_file ='final_consolidated_rearrangements_high_conf_T1.txt' 
#bg_file = '/work/isabl/home/gundemg/projects/neuroblastoma/samples/I-H-116988-T1-1-D1-1/brass/I-H-116988-T1-1-D1-1.ngscn.abs_cn.bg.gz'
#bb_file = '/work/isabl/home/gundemg/projects/neuroblastoma/samples/I-H-116988-T1-1-D1-1/bbg/I-H-116988-T1-1-D1-1_subclones.txt'
#baf_file = '/work/isabl/home/gundemg/projects/neuroblastoma/samples/I-H-116988-T1-1-D1-1/bbg/I-H-116988-T1-1-D1-1_logR_Baf_segmented.vcf.gz'
#legend_pos = 'topright'
#rearr_col = 'custom'
#bbg_col = 'custom'
#image_type = 'png'
...
#cn_win_size = 1e5
#yrange = NULL
#outfile = 'I-H-116988-T1-1-D1-1__genome_wide.png'
#flip_higher = TRUE
...
bb_segments = NULL
bb_segments = read.table(file=bb_file, header=T, comment.char="", sep="\t", stringsAsFactors=F)
bb_segments$length = bb_segments$endpos - bb_segments$startpos
bb_segments = correct_clonality(bb_segments,sf_cutoff=0.11)
cat(paste('Read BBG segments...', '\n',sep=''))
neutral_cns = c(1,1)
...
} else if (bbg_col=="custom") {
	for(color in unique(bb_segments$color)) {
		bbg_sub = bb_segments[bb_segments$color==color,]
		for(k in 1:nrow(bbg_sub)) {
			sel = baf_data_sorted$Chromosome==bbg_sub$chr[k] & baf_data_sorted$Position>=bbg_sub$startpos[k] & baf_data_sorted$Position<=bbg_sub$endpos[k]
			baf_data_sorted$color[sel] = color
		}
	}
}
...
if(image_type=='pdf') {
	...
	par(mfrow=c(2,1), mai=c(0.025,1,0.82,0.42))
}
if(image_type=='png') {
	png(outfile, 2400, 1200)
	par(mfrow=c(2,1), mai=c(0.025,1,0.82,0.42))
}
# Plot BAF
plot(c(), xlim=c(1,max(baf_data_sorted$plot_position)), ylim=c(0,1), type = "n", xaxt = "n", xlab="", ylab="BAF", yaxs="i", xaxs="i", cex.lab=2)
...
clone_legend = unique(bb_segments[,c('clone','color')])
if(bbg_col=="custom") {
	legend("topleft", legend=as.character(clone_legend$clone), fill=as.character(clone_legend$color), text.font=2)
} else {
	legend("topleft", legend=c('clonal','subclonal','no change'), fill=c('brown','salmon','darkgrey'), text.font=2)
}
dev.off()

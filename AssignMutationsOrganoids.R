...
source("/well/wedge/shared/software/multiDimensionalClustering_DCW.R")
reassignInPackage("multiDimensionalClustering",pkgName="DPClust",multiDimensionalClustering.DCW)

#AssignMutations
source("/well/wedge/shared/software/mutation_assignment_em.DCW.R")
source("/well/wedge/shared/software/AssignMutations.R")
...
for(i in 1:length(subsamplenames)){
	...
	if(i==1){
		...
		normalCopyNumber = DP.info$normalCopyNumber
		...
		normalCopyNumber = cbind(normalCopyNumber,DP.info$normalCopyNumber)
...
normalCopyNumber = normalCopyNumber[-exclude.indices,]
...
#YES, just assign mutations, after running Gibbs sampler
AssignMutations(mutCount = mutCount, WTCount = WTCount, totalCopyNumber = totalCopyNumber, normalCopyNumber = normalCopyNumber, copyNumberAdjustment = copyNumberAdjustment,
...
#switch to method 2 (EM algorithm), because of high dimensionality
#AssignMutations(mutCount = mutCount, WTCount = WTCount, totalCopyNumber = totalCopyNumber, normalCopyNumber = normalCopyNumber, copyNumberAdjustment = copyNumberAdjustment,
#mutation.copy.number = mutation.copy.number, cellularity = cellularity, output_folder = output_folder, no.iters = no.iters, no.iters.burn.in = ceiling(no.iters/5),
#subsamplesrun = subsamplenames,samplename=donor, conc_param = 1, cluster_conc = 5, mut.assignment.type = 2, most.similar.mut = NA, mutationTypes = NA, min.frac.snvs.cluster = NA)

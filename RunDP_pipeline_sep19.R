...
#' 
#' sd11@sanger.ac.uk
#' Modified 'slightly' by gundemg@mskcc.org
#'
...
supported_commands = c('nd_dp', 'replot_1d', 'replot_nd', 'sample_muts', 'reassign_muts_1d')
...
if(analysis_type %in% c('1d_dp','replot_1d','sample_muts')) {
...
} else if (analysis_type %in% c('nd_dp','replot_nd','reassign_muts_1d')) {
	samplename = unique(sample2purity$sample)[run]
	datafiles = sample2purity$datafile
	subsamples = sample2purity$sample
	cellularity = sample2purity$cellularity
} else {
	print(paste(c("Specify one of these analysis types:", supported_commands)), sep=" ")
	q(save="no", status=1)
}

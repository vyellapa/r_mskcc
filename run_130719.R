...
mm=m
mm$I.H.130719.T1.2_cluster=factor(mm$I.H.130719.T1.2_cluster)
mm=melt(mm)
...
clust=unique(clust)
clust$I.H.130719.T1.2_merger=make.unique((clust$I.H.130719.T1.2_merger))
cols=clust[,c(1,2)]
...
co=merge(clust,sidecols, by.x="I.H.130719.T1.2_cluster",by.y="cluster", sort = F)
...
colnames(clust)=c("I719.2.L.Rib10","I719.4.Spine","I719.2.L.Psoas","I719.2.L.Sacrum")

png(file = "I130719_d15.png", res=300, width=2000, height=3000)
heatmap.2(as.matrix(clust), dendrogram="row",Rowv=TRUE,Colv=FALSE, scale="row",distfun=function(clust) dist(clust,method = 'euclidean'), hclustfun= function(clust) hclust(clust,method = 'ward'),colsep=c(1), RowSideColors= sidecols, col = bluered, trace="none", margins = c(12,16), labRow = FALSE)
dev.off()
